...
(function () {
  ...
  /* Состав глав можно задать на ходу — например, когда он зависит от роли
     ученика. Передай массив id страниц: kuSetChapters(["cooking","trainer"]). */
  window.kuSetChapters = function (ids) {
    KU.order = ids.slice();
    KU.pages = ["home"].concat(KU.order);
    KU.names = { home: "" };
    KU.order.forEach((id, i) => KU.names[id] = "Глава " + (i + 1));
    KU.unlocked = Math.min(Math.max(KU.unlocked, 1), KU.order.length);
    applyLocks();
    const active = document.querySelector(".ku-page.active");
    const idx = active ? KU.order.indexOf(active.id.replace("ku-page-", "")) : -1;
    const countEl = document.getElementById("ku-course-count");
    const barEl = document.getElementById("ku-course-bar");
    if (countEl) countEl.textContent = idx === -1 ? "" : (idx + 1) + " / " + KU.order.length;
    if (barEl) barEl.style.width = idx === -1 ? "0%" : Math.round(((idx + 1) / KU.order.length) * 100) + "%";
  };
